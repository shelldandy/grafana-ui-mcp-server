 ...
 */
export function parseMDXContent(componentName: string, mdxCode: string): MDXContent {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (headingMatch) {
      ...
      if (currentSection) {
        ...
        currentSection.examples = extractExamplesFromContent(currentSection.content);
 ...
 */
export function parseMDXMetadata(componentName: string, mdxCode: string): MDXMetadata {
...
export function extractUsagePatterns(mdxCode: string): string[] {
  ...
  for (const section of content.sections) {
    if (section.title.toLowerCase().includes('usage') ||
        section.title.toLowerCase().includes('example') ||
        section.title.toLowerCase().includes('pattern')) {
      
      // Extract key points from the section
      const sentences = section.content.split(/[.!?]+/).map(s => s.trim()).filter(Boolean);
...
export function extractAccessibilityGuidelines(mdxCode: string): string[] {
  ...
  const a11yRegex = /(aria-[\w-]+|role=|screen reader|keyboard|focus|accessible)/gi;
  const matches = mdxCode.match(a11yRegex);
  if (matches) {
    guidelines.push(`Includes accessibility features: ${[...new Set(matches)].join(', ')}`);
  }
